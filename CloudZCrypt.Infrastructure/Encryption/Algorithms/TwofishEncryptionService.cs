...
{
    internal class TwofishEncryptionService : IEncryptionService
    {
        private const int KeySize = 256; // Key size for Twofish (256 bits)
        private const int BlockSize = 128; // Block size for Twofish (128 bits)
        ...
        /// <summary>
        /// Encrypts a file using Twofish-256 with a password
        /// </summary>
        ...
        public async Task<bool> EncryptFileAsync(string sourceFilePath, string destinationFilePath, string password)
        {
            try
            {
                ...
                // Create Twofish engine
                TwofishEngine twofishEngine = new();
                CbcBlockCipher cbcBlockCipher = new(twofishEngine);
