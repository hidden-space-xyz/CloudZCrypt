...
/// ensure arguments are not <c>null</c> or empty unless otherwise noted.
/// </remarks>
public class FileOperationsService : IFileOperationsService
{
    ...
    /// <param name="searchPattern">An optional search pattern (e.g. "*.txt"). Defaults to <c>*.*</c> to include all files.</param>
    ...
    /// <returns>The directory name, or <c>null</c> if the path denotes a root or cannot be resolved.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="filePath"/> is <c>null</c> or empty.</exception>
