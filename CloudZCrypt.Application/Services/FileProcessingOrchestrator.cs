...
public sealed class FileProcessingOrchestrator(
    ...
    IEncryptionServiceFactory encryptionServiceFactory) : IFileProcessingOrchestrator
{
    ...
    private async Task<Result<FileProcessingResult>> ProcessOperationAsync(string sourcePath, string destinationPath, FileProcessingOrchestratorRequest request, IProgress<FileProcessingStatus> progress, CancellationToken cancellationToken)
    {
        ...
        if (!isDirectory && !isFile) return Result<FileProcessingResult>.Failure("Source path does not exist.");
        ...
        if (isFile)
        {
            try
            {
                ...
                long fileSize = 0; try { fileSize = fileOperations.GetFileSize(sourcePath); } catch { }
        ...
        for (int i = 0; i < files.Length; i++)
        {
            ...
            if (!string.IsNullOrEmpty(destDir)) await fileOperations.CreateDirectoryAsync(destDir, cancellationToken);
            try
            {
                bool operationResult = await ProcessSingleFile(encryptionService, file, destinationFilePath, request, cancellationToken);
                if (operationResult) processedFiles++;
            }
            catch (Domain.Exceptions.EncryptionAccessDeniedException ex) { stopwatch.Stop(); return Result<FileProcessingResult>.Failure($"Operation stopped due to access denied error: {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionInsufficientSpaceException ex) { stopwatch.Stop(); return Result<FileProcessingResult>.Failure($"Operation stopped due to insufficient disk space: {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionInvalidPasswordException ex) { stopwatch.Stop(); return Result<FileProcessingResult>.Failure($"Operation stopped due to invalid password: {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionKeyDerivationException ex) { stopwatch.Stop(); return Result<FileProcessingResult>.Failure($"Operation stopped due to key derivation error: {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionFileNotFoundException ex) { errors.Add($"File not found (skipped): {file} - {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionCorruptedFileException ex) { errors.Add($"Corrupted file (skipped): {file} - {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionCipherException ex) { errors.Add($"Cipher error for file: {file} - {ex.Message}"); }
            catch (Domain.Exceptions.EncryptionException ex) { errors.Add($"Encryption error for file: {file} - {ex.Message}"); }
            long fileSize = 0; try { fileSize = fileOperations.GetFileSize(file); } catch { }
        ...
        if (errors.Count > 0 && processedFiles == 0)
            return Result<FileProcessingResult>.Failure($"Failed to process any files. Errors: {string.Join("; ", errors)}");
        return Result<FileProcessingResult>.Success(new FileProcessingResult(isSuccess, stopwatch.Elapsed, totalBytes, processedFiles, files.Length, errors));
    }

    private Task<bool> ProcessSingleFile(IEncryptionAlgorithmStrategy encryptionService, string sourceFile, string destinationFile, FileProcessingOrchestratorRequest request, CancellationToken cancellationToken)
    {
    ...
    { error = null; try { if (string.IsNullOrWhiteSpace(rawPath)) return string.Empty; string expanded = Environment.ExpandEnvironmentVariables(rawPath.Trim()); string full = System.IO.Path.GetFullPath(expanded); return full; } catch (Exception ex) { error = $"Invalid path: {ex.Message}"; return null; } }
    private static string FormatBytes(long bytes) { if (bytes == 0) return "0 B"; string[] suffixes = ["B", "KB", "MB", "GB", "TB"]; double size = Math.Abs(bytes); int suffixIndex = 0; while (size >= 1024 && suffixIndex < suffixes.Length - 1) { size /= 1024; suffixIndex++; } return $"{size:F1} {suffixes[suffixIndex]}"; }
}
